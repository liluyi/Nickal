...
namespace Platform.Core.Services
{
    ...
    internal sealed class PluginsService :IPluginService
    {
        ...
        private void OnDealRequst(object sender, DealRequstArgs args)
        {
            ...
            if (args.Type == PluginRequstType.OpenProject)
            {
                Requst_OpenProject(args as OpenProjectArgs);
            }
        ...
        private void Requst_BuildOtherProject(BuildProjectArgs args)
        {
            ...
            AbstractProjectData projectdata = plugin.GetDefaultProjectData();
            project.PluginUUID = plugin.Token;
            projectdata.PluginUUID = plugin.Token;
            project.Name = args.ProjectName;
            projectdata.Name = args.ProjectName;
            project.Path = args.ProjectPath;
            //projectdata.Path = args.ProjectPath;
            ProjectManager.ProjectManagerSington.InsertProject(project);
            ProjectManager.ProjectManagerSington.InsertProjectData(projectdata);
            //将UI加载移至MainPlugin中实现
        ...
        /// </summary>
        /// <param name="args"></param>
        private void Requst_OpenProject(OpenProjectArgs args)
        {
            if (args == null || args.FromToken != "Main")
            {
                return;
            ...
            IPlugin plugin = PluginsManager.PluginsManagerSington.GetPlugin(args.BuildPluginToken);
            AbstractProject project = args.project;
            AbstractProjectData projectdata = plugin.GetDefaultProjectData();
            projectdata.Solution=project.Solution;
            projectdata.Name=project.Name;
            ProjectManager.ProjectManagerSington.InsertProject(project);
            ProjectManager.ProjectManagerSington.InsertProjectData(projectdata);
            //将UI加载移至MainPlugin实现
            //ServicesManager.ServicesManagerSingleton.UIService.LoadUI_CreateProject(plugin, new ProjectUIArgs(args.project.Name, args.project.Path, plugin.MutableResourceClassFullName, project.UUID));
        }
        ...
        private void Requst_RemoveProject(RemoveProjectArgs args)
        {
            ...
            try
            {
                ...
                ProjectManager.ProjectManagerSington.RemoveProjectData(args.ProjectUUID);
            }
            catch
            {
